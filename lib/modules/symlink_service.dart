...
class SymlinkEntry {
  ...
  bool get hasBackup => backupPath != csvEmptyPlaceholder && backupPath.isNotEmpty;

  @override
  String toString() => 'SymlinkEntry(link=$linkPath, target=$targetPath, status=$status)';
}
...
class SymlinkService {
    ...
    if (!jsonFile.existsSync()) {
      ...
      if (csvFile.existsSync()) {
        ...
        try {
          ...
          _logger.info('Migrated ${migratedEntries.length} entries from CSV to JSON.');
        } catch (e) {
      ...
      final jsonStr = const JsonEncoder.withIndent('  ').convert(
        migratedEntries.map((e) => e.toJson()).toList(),
      );
  ...
  Future<List<SymlinkEntry>> readAllEntries() async {
    ...
    try {
      ...
      return list.map((item) => SymlinkEntry.fromJson(item as Map<String, dynamic>)).toList();
    } catch (e) {
  ...
  Future<void> updateEntryStatus(String linkPath, String oldStatus, String newStatus) async {
    final entries = await readAllEntries();
    bool updated = false;
    for (final entry in entries) {
      if (entry.linkPath.toLowerCase() == linkPath.toLowerCase() && entry.status == oldStatus) {
        entry.status = newStatus;
        updated = true;
        _logger.info('Updated status in JSON: $linkPath from $oldStatus to $newStatus');
      }
    }
    if (updated) {
      await _saveJson(entries);
  ...
  Future<void> fixEntryTarget(String linkPath, String newTargetPath) async {
    ...
    for (int i = 0; i < entries.length; i++) {
      ...
      if (entry.linkPath.toLowerCase() == linkPath.toLowerCase() && entry.status == statusActive) {
  ...
  Future<void> writeLog(String operation, String linkPath, String targetPath, {String? details}) async {
